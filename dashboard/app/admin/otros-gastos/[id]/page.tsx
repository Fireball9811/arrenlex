...
export default function AdminOtrosGastoDetallePage() {
  const { t } = useLang()
  ...
  const handleChangeState = async (newState: string) => {
    ...
    setUpdating(true)
    try {
      const res = await fetch(`/api/otros-gastos/${gasto.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ estado: newState }),
      })
      if (res.ok) {
        const data = (await res.json()) as OtroGastoDetalle
        setGasto(data)
      }
    } finally {
      setUpdating(false)
    ...
    <div>
      <div className="mb-6 flex items-center justify-between no-print">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <option value="pendiente">{t.otrosGastos.estados.pendiente}</option>
            <option value="emitido">{t.otrosGastos.estados.emitido}</option>
            <option value="cancelado">{t.otrosGastos.estados.cancelado}</option>
          </select>
          <Button variant="outline" size="sm" asChild>
            <Link href={`/admin/otros-gastos/${gasto.id}/editar`}>
              <Pencil className="mr-2 h-4 w-4" />
              {t.comun.editar}
            </Link>
          </Button>
          <Button variant="outline" size="sm" asChild>
            <Link href="/admin/otros-gastos">{t.otrosGastos.verHistoricoCompleto}</Link>
          </Button>
        ...
        <CardContent className="p-8">
          <div className="flex items-start justify-between mb-8 border-b pb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-sky-700">ARRENLEX</h2>
          ...
          <div className="border-t pt-6 mb-8">
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-sky-700">{formatValor(gasto.valor)}</p>
              </div>
              {gasto.banco && (
