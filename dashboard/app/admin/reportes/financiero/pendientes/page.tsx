...
export default function AdminPendientesMesPage() {
  ...
  const cargar = (pid: string) => {
    setLoading(true)
    setError(null)
    let url = "/api/propietario/pendientes-mes"
  ...
  return (
    <div suppressHydrationWarning>
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="text-base">Filtrar por propietario</CardTitle>
        </CardHeader>
        <CardContent>
          <select
            value={propietarioId}
        ...
        <Card className="border-green-200 bg-green-50">
          <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <p className="text-sm text-green-600">
              No hay propiedades pendientes en {datos ? getNombreMes(datos.mes) : ""}
            </p>
          </CardContent>
        </Card>
      )}
