...
export function HistorialInquilinosTab() {
  ...
  function eliminarUsuario(inquilino: InquilinoInactivo) {
    if (!confirm(`¿Eliminar arrendatario ${inquilino.nombre}? Esta acción desvinculará el arrendatario de tu cuenta de usuario.`)) return

    console.log("[eliminarUsuario] Intentando eliminar arrendatario:", {
      inquilinoId: inquilino.id,
      nombre: inquilino.nombre,
      user_id: inquilino.user_id
    })
    ...
    fetch(`/api/admin/arrendatarios/${inquilino.id}`, {
      ...
      .then(async (res) => {
        const data = await res.json()
        console.log("[eliminarUsuario] Respuesta:", data)
        if (data.error) alert(data.error)
        else {
