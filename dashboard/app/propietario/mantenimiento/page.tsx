...
export default function PropietarioMantenimientoPage() {
  ...
  const diasDesde = (dateStr: string, fallbackDate?: string) => {
    let fecha = parseLocalDate(dateStr)
    if (!fecha && fallbackDate) fecha = parseLocalDate(fallbackDate)
    if (!fecha) return "—"
    const hoy = new Date()
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">{t.mantenimiento.descripcionAdmin}</p>
      </div>
        ...
        <Card className="mb-6 max-w-2xl">
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <div>
              ...
              <CardDescription>{t.mantenimiento.cardDescAdmin}</CardDescription>
            </div>
      ...
      <Card>
        <CardHeader><CardTitle>{t.mantenimiento.cardTitulo}</CardTitle></CardHeader>
        <CardContent>
          {loading ? (
                          ...
                          <tbody>
                            {filtered.map((s) => (
                              <tr key={s.id} className="border-b">
                                ...
                                <td className="max-w-[200px] truncate p-2" title={refPropiedad(s)}>{refPropiedad(s)}</td>
                                <td className="max-w-[200px] truncate p-2" title={s.detalle}>{s.detalle || "—"}</td>
                                <td className="p-2 whitespace-nowrap">{diasDesde(s.desde_cuando, s.created_at)}</td>
                                ...
                                <td className="p-2">
                                  <Link
                                    href={`/propietario/mantenimiento/${s.id}`}
