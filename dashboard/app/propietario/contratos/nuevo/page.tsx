...
export default function PropietarioNuevoContratoPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      router.push(`/propietario/contratos/${data.id}`)
    } else {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>No hay propiedades disponibles para crear un contrato.</CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild>
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>No hay arrendatarios disponibles. Todos tienen un contrato activo.</CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild><Link href="/nuevo">Nuevo arrendatario</Link></Button>
        </CardContent>
      </Card>
    )
  }
    ...
    <div>
      <div className="mb-6 flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild>
          <Link href="/propietario/contratos"><ArrowLeft className="h-5 w-5" /></Link>
        </Button>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4">
              <div>
                ...
                <select required className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={formData.propiedad_id}
                  onChange={(e) => {
                    const prop = propiedades.find((p) => p.id === e.target.value)
                    setFormData({ ...formData, propiedad_id: e.target.value, canon_mensual: prop?.valor_arriendo || 0, ciudad_firma: prop?.ciudad || "" })
                  }}>
                  {propiedades.map((p) => <option key={p.id} value={p.id}>{p.direccion} · {p.ciudad}</option>)}
                </select>
                <p className="text-xs text-muted-foreground mt-1">Mostrando {propiedades.length} propiedades disponibles</p>
              </div>
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">Mostrando {arrendatarios.length} arrendatarios disponibles</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="date" required className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={formData.fecha_inicio} onChange={(e) => setFormData({ ...formData, fecha_inicio: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Duración (meses) *</label>
                <input type="number" required min="1" max="120" className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={formData.duracion_meses} onChange={(e) => setFormData({ ...formData, duracion_meses: parseInt(e.target.value) || 1 })} />
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">Canon mensual *</label>
            ...
            {selectedPropiedad?.matricula_inmobiliaria && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                <p className="text-sm text-amber-800"><strong>Matrícula inmobiliaria:</strong> {selectedPropiedad.matricula_inmobiliaria}</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <div className="flex gap-2">
          ...
          <Button type="button" variant="outline" asChild>
            <Link href="/propietario/contratos">Cancelar</Link>
          </Button>
        </div>
      </form>
    </div>
