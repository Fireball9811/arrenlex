...
type Propiedad = { id: string; direccion: string; ciudad: string; valor_arriendo: number }
type Arrendatario = { id: string; nombre: string; cedula: string }

export default function PropietarioEditarContratoPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      router.push(`/propietario/contratos/${params.id}`)
    } else {
  ...
  if (!contrato) {
    return (
      <Card>
        ...
        <CardContent>
          <Button asChild><Link href="/propietario/contratos">Volver a contratos</Link></Button>
        </CardContent>
      </Card>
    )
  }
    ...
    <div>
      <div className="mb-6 flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild>
          <Link href={`/propietario/contratos/${params.id}`}><ArrowLeft className="h-5 w-5" /></Link>
        </Button>
      ...
      <div className="space-y-6">
                ...
                <div>
                  ...
                  <select required className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={formData.propiedad_id}
                    onChange={(e) => {
                      ...
                      setFormData({ ...formData, propiedad_id: e.target.value, canon_mensual: prop?.valor_arriendo || formData.canon_mensual })
                    }}>
        ...
        <DocumentosContrato contratoId={params.id as string} puedeEditar={true} />
        <RecibosContrato contratoId={params.id as string} puedeEditar={true} />
      </div>
    </div>
  )
}
