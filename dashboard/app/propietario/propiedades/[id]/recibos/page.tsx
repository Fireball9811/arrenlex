...
import { ArrowLeft, Receipt, Plus, Eye, Edit2, Trash2, FileText } from "lucide-react"
...
export default function RecibosDePropiedad() {
  ...
  const handleEliminar = async (reciboId: string) => {
    ...
    try {
      const res = await fetch(`/api/recibos-pago/${reciboId}`, { method: "DELETE" })
  ...
  return (
    <div suppressHydrationWarning>
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <Link href={`/propietario/recibos/nuevo?propiedad_id=${propiedadId}`}>
          <Button>
      ...
      {recibos.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground font-semibold">No hay recibos de pago para esta propiedad</p>
            <p className="text-muted-foreground text-sm mt-1">Crea tu primer recibo de pago</p>
            <Link href={`/propietario/recibos/nuevo?propiedad_id=${propiedadId}`}>
        ...
        <div className="space-y-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground grid grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {formatCalendarDateEs(recibo.fecha_recibo)}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="text-sm text-muted-foreground mt-6 p-4 bg-muted rounded-lg">
            <p className="font-semibold mb-1">Total emitido</p>
            <p className="text-lg font-bold text-green-600">
              {new Intl.NumberFormat("es-CO", {
                style: "currency",
                currency: "COP",
                maximumFractionDigits: 0,
              }).format(recibos.reduce((sum, r) => sum + (r.valor_arriendo || 0), 0))}
            </p>
          </div>
        </div>
      )}
