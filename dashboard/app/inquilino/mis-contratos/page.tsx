...
type Contrato = {
  id: string
  propiedades: { direccion: string; ciudad: string; barrio: string; id?: string }
  arrendatarios: { nombre: string; cedula: string }
  fecha_inicio: string
  fecha_fin: string
    ...
    const fetchContratos = async () => {
      ...
      if (user) {
        const { data } = await supabase
          .from("contratos")
          .select(`*, propiedades (direccion, ciudad, barrio), arrendatarios (nombre, cedula)`)
            ...
            <Card key={c.id}>
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`rounded px-2 py-0.5 text-xs font-medium ${estadoColors[c.estado] ?? "bg-gray-100 text-gray-800"}`}>{estadoLabels[c.estado] || c.estado}</span>
                </div>
                <CardDescription>{c.propiedades?.ciudad} · {c.propiedades?.barrio}</CardDescription>
              </CardHeader>
